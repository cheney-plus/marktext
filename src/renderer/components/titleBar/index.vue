  ...
  <div>
    ...
    >
      <div class="title" @dblclick.stop="toggleMaxmizeOnMacOS">
        <span v-if="!filename">markup</span>
        <span v-else>
          <span
      ...
      <div :class="showCustomTitleBar ? 'left-toolbar title-no-drag' : 'right-toolbar'">
        ...
        <el-tooltip
          v-if="wordCount"
          class="item"
          :content="`${wordCount[show]} ${wordCountLabels[show].full}`"
        ...
        >
          ...
          >
            <span class="text-center-vertical">{{ `${wordCountLabels[show].short} ${wordCount[show]}` }}</span>
          </div>
        </el-tooltip>
      </div>
      <div
...
export default {
  ...
  computed: {
    ...
    wordCountLabels () {
      ...
      return {
        ...
        character: {
          short: useZh ? '字符' : 'C',
          full: this.$t('Characters')
        },
        paragraph: {
          short: useZh ? '段落' : 'P',
          full: this.$t('Paragraphs')
        },
        all: {
          short: useZh ? '字符(含空格)' : 'A',
          full: this.$t('Characters (with spaces)')
        }
      }
    }
  },
  watch: {
    filename: function (value) {
      ...
      let title = ''
      if (value) {
        title = hasOpenFolder ? `${value} - ${this.project.name}` : `${value} - markup`
      } else {
        title = hasOpenFolder ? this.project.name : 'markup'
      }
