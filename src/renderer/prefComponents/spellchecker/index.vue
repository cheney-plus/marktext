<template>
  <div class="pref-spellchecker">
    <h4>{{ $t('Spellchecker') }}</h4>
    <compound>
      <template #head>
        <bool
          :description="$t('Enable spellchecker')"
      ...
      <template #children>
        ...
        <bool
          v-show="isOsx"
          :description="$t('Automatically detect document language')"
          :bool="true"
          :disable="true"
        ></bool>
        <cur-select
          v-show="!isOsx"
          :description="$t('Default language for spell checking')"
    ...
    <div v-if="isOsx && spellcheckerEnabled" class="description">
      {{ $t('The used language will be detected automatically while typing. Additional languages may be added through "Language & Region" in your system preferences pane.') }}
    </div>

    <div v-if="!isOsx && spellcheckerEnabled">
      <h6 class="title">{{ $t('Custom dictionary:') }}</h6>
      <div class="description">{{ $t('Edit words in custom dictionary.') }}</div>
      <el-table
        :data="wordsInCustomDictionary"
        :empty-text="$t('No words available')"
        style="width: 100%"
      >
        <el-table-column prop="word" :label="$t('Word')">
        </el-table-column>

        <el-table-column fixed="right" :label="$t('Options')" width="90">
          <template slot-scope="scope">
            <el-button @click="handleDeleteClick(scope.row)" type="text" size="small" :title="$t('Delete')">
    ...
    handleSpellcheckerLanguage (languageCode) {
      this.ensureDictLanguage(languageCode)
        ...
        .catch(error => {
          log.error(error)
          notice.notify({
            title: this.$t('Failed to switch language'),
            type: 'error',
        ...
        ipcRenderer.invoke('mt::spellchecker-remove-word', selectedItem.word)
          .then(success => {
            if (success) {
              ...
              notice.notify({
                title: this.$t('Failed to remove custom word'),
                type: 'error',
