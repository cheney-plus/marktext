<template>
  <section class="image-folder">
    <h5>{{ $t('Global or relative image folder') }}</h5>
    <text-box :description="$t('Global image folder')" :input="imageFolderPath"
      :regexValidator="/^(?:$|([a-zA-Z]:)?[\/\\].*$)/" :defaultValue="folderPathPlaceholder"
      :onChange="value => modifyImageFolderPath(value)"></text-box>
    <div>
      <el-button size="mini" @click="modifyImageFolderPath(undefined)">{{ $t('Open...') }}</el-button>
      <el-button size="mini" @click="openImageFolder">{{ $t('Show in Folder') }}</el-button>
    </div>
    <compound>
      <template #head>
        <bool :description="$t('Prefer relative assets folder')"
      ...
      <template #children>
        <text-box :description="$t('Relative image folder name')" :input="imageRelativeDirectoryName"
          :regexValidator="/^(?:$|(?![a-zA-Z]:)[^\/\\].*$)/"
          :defaultValue="relativeDirectoryNamePlaceholder"
          :onChange="value => onSelectChange('imageRelativeDirectoryName', value)"></text-box>
        <div class="footnote">
          {{ $t('Include {filename} in the text-box above to automatically insert the document file name.') }}
        </div>
      </template>
    </compound>
  </section>
