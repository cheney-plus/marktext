  ...
  <div class="pref-keybindings">
    ...
    <section class="keybindings">
      ...
      >
        ...
        <el-table-column prop="accelerator" :label="$t('Key Combination')" width="220">
        </el-table-column>
        <el-table-column fixed="right" :label="$t('Options')" width="90">
          <template slot-scope="scope">
            <el-button @click="handleEditClick(scope.$index, scope.row)" type="text" size="small" :title="$t('Edit')">
              <i class="el-icon-edit"></i>
            </el-button>
            <el-button @click="handleResetClick(scope.$index, scope.row)" type="text" size="small" :title="$t('Reset')">
              <i class="el-icon-refresh-right"></i>
            </el-button>
            <el-button @click="handleUnbindClick(scope.$index, scope.row)" type="text" size="small" :title="$t('Unbind')">
    ...
    <section class="footer">
      <separator></separator>
      <el-button size="medium" @click="saveKeybindings">{{ $t('Save') }}</el-button>
      <el-button size="medium" @click="restoreDefaults">{{ $t('Restore default key bindings') }}</el-button>
    </section>
    <section v-if="showDebugTools" class="keyboard-debug">
      <separator></separator>
      <div><strong>{{ $t('Debug options:') }}</strong></div>
      <el-button size="medium" @click="dumpKeyboardInformation">{{ $t('Dump keyboard information') }}</el-button>
    </section>
    <key-input-dialog
...
export default {
  ...
  methods: {
      ...
      if (this.keybindingConfigurator && this.keybindingList.length > 0) {
        this.keybindingConfigurator.save()
          .then(success => {
            if (!success) {
              notice.notify({
                title: this.$t('Failed to save'),
                type: 'error',
                message: this.$t('An unexpected error occurred while saving.')
              })
            }
          })
    ...
    handleDuplicateShortcut (id, accelerator) {
      notice.notify({
        title: this.$t('Shortcut already in use'),
        type: 'warning',
        message: this.$t('The shortcut is already in use. Please unset the shortcut and try again.').replace('{shortcut}', accelerator)
      })
    },
